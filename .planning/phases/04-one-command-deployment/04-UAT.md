...
number: 1
name: Browser-tab wall-clock shutdown confirmation
expected: |
  With `finally-app` running and a real browser tab open on `http://localhost:8000`
  (genuine `EventSource`, not curl), run `docker stop --timeout 15 finally-app` and
  time it. Returns within roughly 10-15 seconds, no manual force-kill needed.
