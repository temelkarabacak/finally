...
class TestFactory:
    ...
    def test_creates_massive_when_api_key_set(self):
        """Test that the factory wraps Massive in FailoverMarketDataSource."""
        cache = PriceCache()
    ...
    def test_massive_receives_api_key(self):
        """Test that the wrapped Massive client receives the API key."""
        cache = PriceCache()
        ...
        assert isinstance(source, FailoverMarketDataSource)
        assert isinstance(source.active, MassiveDataSource)
        assert source.active._api_key == "test-key-123"
    ...
    def test_massive_receives_cache(self):
        """Test that the wrapped Massive client receives the cache reference."""
        cache = PriceCache()
        ...
        assert isinstance(source, FailoverMarketDataSource)
        assert isinstance(source.active, MassiveDataSource)
        assert source.active._cache is cache
